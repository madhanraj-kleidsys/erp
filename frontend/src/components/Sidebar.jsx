...
export default function Sidebar({ user, onLogout }) {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/api/approvalCount')  // use your actual API path
      .then(({ data }) => {
        setApprovalData({
          codes: data.codes,
          counts: data.count,
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
        ...
        />
        {/* </IconButton> */}
        <Typography level="title-lg">Madhanraj</Typography>
      ...
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
        ...
        <Box sx={{ minWidth: 0, flex: 1 }}>
          ...
          <Typography level="title-sm">{user?.name || 'Madhanraj'}</Typography>

          <Typography level="body-xs">madhan@gmail.com</Typography>
        </Box>
